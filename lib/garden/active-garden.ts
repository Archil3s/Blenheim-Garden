...
export const SEASONAL_PLAN_EVENT = "blenheim-garden-seasonal-plan-generated";

export function gardenLocalPlanKey(gardenId: string) {
  return gardenId === DEFAULT_GARDEN_ID
    ? LOCAL_PLAN_PREFIX
    : `${LOCAL_PLAN_PREFIX}:${gardenId}`;
}

export function gardenLivePlanKey(gardenId: string) {
  return gardenId === DEFAULT_GARDEN_ID
    ? LIVE_PLAN_PREFIX
    : `${LIVE_PLAN_PREFIX}:${gardenId}`;
}
