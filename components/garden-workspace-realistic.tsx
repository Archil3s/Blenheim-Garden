...
import { SEASONAL_PLAN_EVENT } from "@/lib/garden/active-garden";

import { surfaceMaterial } from "./garden-scene-artwork";
...
const DEMO_INSPECTOR: InspectItem = {
...
function clearGroup(group: THREE.Group) {
  group.traverse((object) => object.userData.disposeDemo?.());
  disposeObject(group);
  group.clear();
}
...
function leaf(color = COLORS.leaf, scale = 1) {
  const geometry = new THREE.PlaneGeometry(1, 1, 4, 10);
  ...
  for (let index = 0; index < positions.count; index += 1) {
    ...
    positions.setXYZ(
      index,
      across * 0.24 * scale * taper * serration,
      t * 0.32 * scale,
      Math.sin(Math.PI * t) * 0.055 * scale +
        (1 - edge) * 0.026 * scale -
        edge * 0.008 * scale,
    );
    const vertexColor = baseColor
      .clone()
      .offsetHSL(0, 0.025 * (1 - edge), 0.08 * (1 - edge) - 0.035 * edge);
    colors.push(vertexColor.r, vertexColor.g, vertexColor.b);
  }
  geometry.setAttribute("color", new THREE.Float32BufferAttribute(colors, 3));
  geometry.computeVertexNormals();
  const leafMaterial = material(0xffffff, 0.64);
...
function fruit(
  ...
  const mesh = new THREE.Mesh(
    new THREE.SphereGeometry(radius, 18, 13),
    material(color, 0.58),
  );
...
function animateCropModel(root: THREE.Group, seed: number) {
  const phase = (seed % 97) * 0.31;
  root.rotation.y = (seed * 0.618) % (Math.PI * 2);
  root.userData.updateArtwork = () => {
    const now = performance.now();
    root.rotation.z = Math.sin(now * 0.0012 + phase) * 0.028;
    root.rotation.x = Math.cos(now * 0.0009 + phase) * 0.01;
  };
}

function plantModel(crop: string, mobile: boolean, seed: number) {
  ...
  if (kind === "tomato") {
    ...
    root.add(
      fruit(COLORS.pumpkin, 0.125, 0.14, 0.105, 0.04, [1.28, 0.72, 1.12]),
    );
    ...
    for (let i = 0; i < 7; i += 1) {
      const blade = new THREE.Mesh(
        new THREE.ConeGeometry(0.014, 0.25, 6),
        material(COLORS.leafDark, 0.86),
      );
    ...
    const shoulder = new THREE.Mesh(
      new THREE.ConeGeometry(0.036, 0.095, 14),
      material(COLORS.carrot, 0.65),
    );
    ...
    const pepper = new THREE.Mesh(
      new THREE.CapsuleGeometry(0.036, 0.1, 4, 8),
      material(0xc73f35, 0.58),
    );
...
function addBed(
  group: THREE.Group,
  bed: PlannerBed,
  active: PlannerPlantingArea | undefined,
) {
  ...
  const soil = new THREE.Mesh(
    new THREE.BoxGeometry(
      Math.max(0.05, width - 0.12),
      0.18,
      Math.max(0.05, depth - 0.12),
    ),
    surfaceMaterial("soil", COLORS.soilTop),
  );
  soil.position.set(x, 0.21, z);
  root.add(soil);
  ...
  rails.forEach(([w, d, px, pz], index) => {
    ...
    const cap = new THREE.Mesh(
      new THREE.BoxGeometry(w + 0.045, 0.045, d + 0.045),
      surfaceMaterial("wood", index % 2 ? 0xa9734b : 0x89583b),
    );
    cap.position.set(px, wallHeight + 0.018, pz);
    root.add(cap);
  });
  ...
  inspectable(root, {
    title: bed.name,
    subtitle: active
      ? `${active.crop}${active.variety ? ` · ${active.variety}` : ""}`
      : "Raised garden bed",
    lines: [
      { label: "Size", value: `${width.toFixed(1)} × ${depth.toFixed(1)} m` },
      ...(active
        ? [{ label: "Spacing", value: `${active.spacingCm} cm` }]
        : []),
    ],
  });
  group.add(root);
...
function addPlanting(
  ...
  for (const point of representativePositions(aw, ah, area.count, mobile)) {
    const seed = area.bedId + point.x * 11 + point.y * 31;
    const plant = plantModel(area.crop, mobile, seed);
    plant.position.set(
      worldX(ax + aw * point.x),
      0.32,
      worldZ(az + ah * point.y),
    );
    plant.scale.setScalar(
      Math.max(0.72, Math.min(1.12, (area.iconSize || 16) / 16)),
    );
    root.add(plant);
  }
...
function addPath(
  group: THREE.Group,
  object: Extract<PlannerPlan["objects"][number], { type: "path" }>,
) {
  ...
  inspectable(root, {
    ...
    lines: [{ label: "Width", value: `${object.widthCm} cm` }],
  });
  group.add(root);
}
...
function addTrellis(
  group: THREE.Group,
  object: Extract<PlannerPlan["objects"][number], { type: "trellis" }>,
) {
  const x1 = worldX(object.x1);
  const z1 = worldZ(object.y1);
  const x2 = worldX(object.x2);
  ...
  const posts = Math.max(
    ...
    Math.min(
      10,
      Math.ceil((length * 100) / Math.max(50, object.postSpacingCm)) + 1,
    ),
  );
  ...
  for (let i = 0; i < posts; i += 1) {
    ...
    const post = new THREE.Mesh(
      new THREE.CylinderGeometry(0.028, 0.038, height, 8),
      material(0x6c5038, 0.78),
    );
  ...
  for (const y of [height * 0.28, height * 0.5, height * 0.72, height * 0.92]) {
    const wire = new THREE.Mesh(
      new THREE.BoxGeometry(length, 0.012, 0.012),
      material(0x727a76, 0.42, 0.18),
    );
  ...
  inspectable(root, {
    ...
    lines: [{ label: "Height", value: `${height.toFixed(1)} m` }],
  });
  group.add(root);
}
...
function addTree(
  group: THREE.Group,
  object: Extract<PlannerPlan["objects"][number], { type: "tree" }>,
  mobile: boolean,
) {
  const root = new THREE.Group();
  ...
  const trunk = new THREE.Mesh(
    new THREE.CylinderGeometry(0.08, 0.13, 0.86, 10),
    material(0x705038, 0.92),
  );
  ...
  for (let i = 0; i < crowns; i += 1) {
    ...
    const crown = new THREE.Mesh(
      new THREE.IcosahedronGeometry(radius * 0.6, mobile ? 1 : 2),
      material(i % 2 ? 0x477c49 : 0x3c7042, 0.92),
    );
    crown.scale.y = 0.82;
    crown.position.set(
  ...
  inspectable(root, {
    ...
    lines: [
      { label: "Canopy", value: `${(object.diameterCm / 100).toFixed(1)} m` },
    ],
  });
  group.add(root);
}
...
function buildGarden(group: THREE.Group, plan: PlannerPlan, mobile: boolean) {
  ...
  plan.beds.forEach((bed) =>
    addBed(
      group,
      bed,
      plan.plantingAreas.find((area) => area.bedId === bed.id),
    ),
  );
  ...
  for (const row of plan.rows) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const plant = plantModel(row.crop, mobile, i + row.spacingCm);
    ...
    inspectable(root, {
      ...
      lines: [{ label: "Spacing", value: `${row.spacingCm} cm` }],
    });
    group.add(root);
  }
...
export function GardenWorkspaceRealistic({ plan }: { plan: PlannerPlan }) {
  ...
  const structureCount = plan.objects.filter(
    (object) => object.type === "structure",
  ).length;

  useEffect(() => {
    const showLivePlan = () => setShowDemo(false);
    window.addEventListener(SEASONAL_PLAN_EVENT, showLivePlan);
    return () => window.removeEventListener(SEASONAL_PLAN_EVENT, showLivePlan);
  }, []);
  ...
  useEffect(() => {
    ...
    setInspector(showDemo ? DEMO_INSPECTOR : DEFAULT_INSPECTOR);
    clearGroup(runtime.content);
    if (showDemo) {
      addDemonstrationBed3D(runtime.content, runtime.mobile);
      runtime.camera.position.set(2.85, 2.75, 4.75);
      runtime.controls.target.set(0, 0.55, 0.12);
    } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      queueMicrotask(() =>
        setRenderError("3D simulator needs WebGL2 on this device."),
      );
      return;
    }
    ...
    renderer.setPixelRatio(
      Math.min(window.devicePixelRatio || 1, mobile ? 1.25 : 1.8),
    );
    ...
    const outer = new THREE.Mesh(
      ...
      surfaceMaterial("grass", COLORS.grassDark),
    );
    ...
    const ground = new THREE.Mesh(
      new THREE.PlaneGeometry(9.5, 11.3),
      surfaceMaterial("grass", COLORS.grass),
    );
    ...
    const onPointerUp = (event: PointerEvent) => {
      ...
      for (const hit of hits) {
        ...
        while (current) {
          ...
          const root = current.userData.selectionRoot as
            THREE.Object3D | undefined;
          if (item && root) {
            clearSelection(selectionRef);
            if (!root.userData.hideSelectionBox) {
    ...
    return () => {
      ...
      if (Array.isArray(grassTufts.material))
        grassTufts.material.forEach((item) => item.dispose());
  ...
  return (
    <div
      className={`gv-3d-workspace gv-3d-realistic${showDemo ? " gv-3d-demo-active" : ""}`}
      data-testid="inline-3d-workspace"
    >
      <div
        className="gv-3d-workspace-canvas"
        ref={mountRef}
        aria-label="Interactive 3D garden workspace"
      />
      {renderError && (
        <div className="gv-3d-workspace-error">{renderError}</div>
      )}
      <div className="gv-3d-hud gv-3d-hud-left">
        ...
        <small>
          {showDemo
            ? "2 × 4 m benchmark"
            : `${structureCount} ${structureCount === 1 ? "structure" : "structures"} · Live plan`}
        </small>
      </div>
      <div
        className="gv-3d-hud gv-3d-camera-controls"
      ...
      >
        ...
        <button type="button" onClick={setPerspective}>
          Fit
        </button>
        <button
          type="button"
      ...
      <div className="gv-3d-help">
        Drag to orbit · wheel/pinch to zoom · tap to inspect
      </div>
    </div>
  );
}
